...
const TimelineSection: React.FC<ShowcaseProps> = ({ creatures, showPortal, onClosePortal }) => {
    ...
    const filterCategories = [
        'DINOSAURS', 'PTEROSAURS', 'CROCODILIANS', 'TURTLES AND TORTOISES', 
        // ... rest of categories
    ];
    const [selectedCategory, setSelectedCategory] = React.useState('PTEROSAURS');
...
return (
        ...
        <section className="bg-gray-900 min-h-screen text-white relative"> 
            ...
            {showPortal && (
                ...
                >
                    <div className="max-w-3xl text-center p-10 bg-gray-800/90 rounded-xl shadow-2xl border-4 border-green-500">
                       <p className="text-2xl font-light leading-relaxed text-gray-200">
                            You have successfully transported to the Mesozoic Era. <br />
                            Brace yourself as you explore the ancient world and exit the portal 
                            with newly found knowledge and virtual experience.
                        </p>
                        
                        <button 
                            onClick={onClosePortal}
                            className="mt-8 px-10 py-3 text-xl font-bold bg-yellow-500 rounded-lg hover:bg-yellow-600 transition duration-300 transform hover:scale-105"
                        >
                            Got It
                        </button>
                    </div>
            ...
            <div className={`flex transition-opacity duration-1000 ${mapOpacity}`}>
